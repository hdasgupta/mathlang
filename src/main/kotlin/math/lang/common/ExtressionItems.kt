...
enum class Operators(val symbol: String, val reduce: (numbers:List<Double>, isReal:Boolean)->String, val operandCount: Int = -1) {
    add("+", {numbers, isReal->
        run {
...
class Operation(@NotNull operator: Operators, @NotNull vararg operands: Operand) : Operand(false) {
    ...
    override fun calc(approx: Boolean): Number {
        ...
        val dnumbers:List<Double> = numbers.map {java.lang.Double.parseDouble(if(it.toString().contains(".")) it.toString() else "${it.toString()}.${"0".repeat(200)}") }
        val result = operator.reduce(dnumbers, isReal)

        return if(isReal || operator==Operators.div) java.lang.Double.parseDouble(result) else java.lang.Integer.parseInt(result)
    }
