...
fun getAllSimplificationEquation(): MutableList<SimplificationFormula> {
    ...
    list.add(
        SimplificationFormula(
            "Convert to Addition from Subtraction",
            { ExpressionConstants.sub(x, y) },
            { applySub(it.operands.toList()) })
    )
    ...
    list.add(
        SimplificationFormula(
            "Communicative Addition",
            { add(y.new(), add(x, y)) },
            { applyCommAdd(it.operands.toList()) })
    )
    ...
    list.add(
        SimplificationFormula(
            ...
            { mul(y.new(), mul(x, y)) },
            { applyCommMul(it.operands.toList()) })
    )
    ...
    list.add(
        SimplificationFormula(
            ...
            { filterMulAdd(it) },
            { applyMulAdd(it.operands.toList()) })
    )
    ...
    list.add(
        SimplificationFormula(
            "Power of Power",
            { ExpressionConstants.pow(x.new(), pow(x, y)) },
            { applyPowPow(it.operands.toList()) })
    )

    list.add(
        SimplificationFormula(
            "Power from Multiplication",
            { ExpressionConstants.mul(x, y) },
            { applyPowMul(it.operands.toList()) })
    )

    list.add(SimplificationFormula("Simplify Addition", { add(x, y) }, {
        applySimAdd(
            it.operands.toList()
        )
    }))

    list.add(SimplificationFormula("Simplify Power, Multiplication and Division", { div(x, pow(x.new(), y.new())) }, {
        applyDivPowMul(
            it.operands.toList()
        )
    }))

    list.add(SimplificationFormula("Simplify Power, Multiplication and Division", { mul(x, pow(x.new(), y.new())) }, {
        applyDivPowMul(
            it.operands.toList()
        )
    }))

    list.add(SimplificationFormula("Simplify Power, Multiplication and Division", { mul(x, div(x.new(), y.new())) }, {
        applyDivPowMul(
            it.operands.toList()
        )
    }))
...
fun getFlatOperands(operation: Operation): List<Operand> {
    return operation.operands.flatMap {
        if (it is Operation && it.operator == operation.operator) getFlatOperands(it) else listOf(
            it
        )
    }
}
...
fun applyMulAdd(it: List<out Operand>): Operand {
    ...
    return add(*adds.reduce { a, o -> dot(a, o, exceptAdds) }.operands.map { ad ->
        when (exceptAdds.size) {
            0 -> ad
...
fun applyPowMul(it: List<out Operand>): Operand {
    ...
    return map.keys.map {
        if (map[it] == null) it else if (getConst<Int>(map[it]!!) != 1) map[it]
            ?.let { it2 ->
                pow(
                    it,
                    it2
    ...
    }.toTypedArray().let { it3 -> mul(*it3 as Array<out Operand>) }
}
...
fun applySimAdd(list: List<out Operand>): Operand {
    ...
    }.let { it3 -> add(*it3.toTypedArray() as Array<out Operand>) }
...
fun applyDivPowMul(list: List<out Operand>): Operand {
    ...
    val invertsPow =
        inverts.filter { it.operands[1] is Operation && (it.operands[1] as Operation).operator == Operators.pow }
    ...
    }.let { it3 -> mul(*it3.toTypedArray() as Array<out Operand>) }
...
open class Formula<out T : Operand, out U>(
    val name: String?,
    val fx: (v: List<@UnsafeVariance T>) -> Operand,
    val dFx: (v: @UnsafeVariance U) -> Operand
)

open class SimplificationFormula(
    name: String?,
    fx: (v: List<out Operand>) -> Operand,
    dFx: (operation: Operation) -> Operand,
    val firstPriority: Boolean = false
) : Formula<Operand, Operation>(name, fx, dFx) {
    ...
    private fun getLevel(f: Operand, i: Int): Map<Int, List<Operators>> {
        ...
        if (f is Operation) {
            ...
            f.operands.map { getLevel(it, i + 1) }.forEach {
                it.keys.forEach { key ->
                    if (map.containsKey(key)) it[key]?.let { it1 ->
                        map[key]?.addAll(
                            it1
                        )
...
open class Result(
    ...
    val assumptions: List<Operation> = listOf(),
    val derive: List<Derive> = listOf()
) {
