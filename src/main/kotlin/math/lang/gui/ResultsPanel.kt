...
class ResultsPanel(private val results: Results) : JTable(results.map { result ->
    listOf(
        result.operand,
        Differentiate(operand = result.fx),
        result.dFx,
        result.formulaName,
        result.assumptions
    ).toTypedArray()
}.toTypedArray(), listOf("result", "d(formula)/dx", "formula out", "Formula name", "assumption").toTypedArray()) {
    init {
...
class OperandCellEditor(val center: Boolean = false) : TableCellEditor {
    ...
    override fun getTableCellEditorComponent(
        ...
        return if (value != null) JScrollPane(
            OperandPanel(value as Operand, center),
